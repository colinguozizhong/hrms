					...
					$.ajax({
						url: '<%=basePath%>deptorg/deleteOrg.do?businessId=' + businessId,
								success:function(){
									...
									//插入删除组织机构日志
									insertSysLog("组织机构管理","删除","名称为"+rows[0].orgName+"的组织机构");
								}
					});
				}
			})
...
//提交编辑
				...
				$.ajax({
					...
					success: function(data){
						if(data == 0){
							...
							//插入新增组织机构日志
							insertSysLog("组织机构管理","修改","名称为"+$("#orgName").textbox("getValue")+"的组织机构");
							$('#dgBusiness').datagrid('reload');
							$("#zTreeMenuBusiness").tree("reload");
				...
				$.ajax({
					...
					success: function(data){
						if(data == 0){
							...
							//插入新增组织机构日志
							insertSysLog("组织机构管理","新增","名称为"+$("#orgName").textbox("getValue")+"的组织机构");
							$('#dgBusiness').datagrid('reload');
							$("#zTreeMenuBusiness").tree("reload");
							//上级管理机构和业务机构重载，避免编辑后显示还是初始化机构树
							$('#orgManage').combotree("reload");
	...
	<body>
	   <div class="easyui-layout" style="width:100%;height:600px;" data-options="fit:true">
	   	   <div data-options="region:'west'" title="组织列表" style="width:20%">
	   			<div >
	   		...
	   		<div data-options="region:'center',title:'组织管理',iconCls:'icon-ok'" >
	   			<div class="zu_zhi_guan_li">
				    <div id="sear"  data-options="region:'north',border:false" style="text-align:left;padding:5px 0 0 15px;">
				    	...
				    	<a name="reSearch" id="reSearch" class="easyui-linkbutton" data-options="iconCls:'icon-reload'" style="width:68px;margin-left:5px">重置</a>
				    </div>
				    <div data-options="region:'center',border:false" style="text-align:left;padding:5px 0 0 5px;">
						<table id="dgBusiness" style="display:block;width:100%">  
		     			</table>
		     		</div>
				</div>
	   		</div>
   </div>
	<div id="dlg_edit" class="easyui-dialog" closed="true" title="组织机构" style="width: 780px; height: 360px; padding: 10px"data-options="buttons: '#dlg-buttons', modal:true">
	 	<form action="" id="orgForm" name="orgForm">
	 	 	<div style="float: left;">
	 	 			<div id="pan" class="easyui-panel" style="width: 350px; height: auto;padding:0 0 0 10px">
						...
						<div style="padding-bottom: 5px">
							<input  id="orgName"  name="orgName" class="easyui-textbox" data-options="label:'组织机构名称:',required:true" style="width:300px"/><span style="color:red;"> * </span>
						</div>
						<div style="padding-bottom: 5px">
							<select  class="easyui-combobox" id="orgLv" name="orgLv"  label="单位级别:" labelPosition="right" style="width:300px" data-options="editable:false,panelHeight:'auto'" >
		    ...
		    <div style="float: left">
		    	<div id="pan2" class="easyui-panel" style="width: 350px; height: auto ;padding:5px 0 0 10px">
				    <div style="padding-bottom: 5px">
			    		<select class="easyui-combobox"  id="bh" name="bh" label="单位主体类别:" labelPosition="right" style="width:300px" data-options="editable:false,panelHeight:'auto'" >
